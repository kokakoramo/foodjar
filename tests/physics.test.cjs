...
test('14 food bodies settle inside jar without material overlap',()=>{
 ...
 for(const b of bodies){assert.ok(b.bounds.min.x>-.5);assert.ok(b.bounds.max.x<262.5);assert.ok(b.bounds.max.y<304.5);assert.ok(b.bounds.min.y>=0)}
...
test('only newly saved food starts above mouth; drag uses scaled coordinates and does not open detail',()=>{
 ...
 const constraint=Matter.Composite.allConstraints(s.engine().world)[0];assert.equal(constraint.pointA.x,180);assert.equal(constraint.pointA.y,100);
