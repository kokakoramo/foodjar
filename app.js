(() => {
  ...
  const defaultState = () => ({
    ...
    meals:[
      {id:'demo-1',date:'2026-09-05',title:'雞腿便當',type:'午餐',price:120,payment:'現金',note:'今天的雞腿很香 ♡',image:'assets/demo-sticker.png',sticker:true},
  ...
  function loadState(){
    try{const raw=safeLSGet(LS_KEY);if(raw){const parsed=JSON.parse(raw);if(parsed?.profile&&Array.isArray(parsed.meals))return parsed}}catch{}
    return defaultState();
  }
  function persist(){safeLSSet(LS_KEY,JSON.stringify(state))}
  ...
  function renderAll(){
    ...
    $('#profileEmail').textContent=session?.user?.email||state.profile.email||'preview@foodjar.local';
    $('#jarNameInput').value=state.profile.jarName;$('#budgetInput').value=state.profile.budget;
  ...
  function renderHome(){
    const ms=mealsForMonth(), total=monthTotal(), budget=Number(state.profile.budget)||1, pct=Math.min(100,total/budget*100), avg=ms.length?total/ms.length:0;
  ...
  function renderRecap(){
    const ms=mealsForMonth(),total=monthTotal(),avg=ms.length?total/ms.length:0,max=ms.length?Math.max(...ms.map(m=>Number(m.price||0))):0,budget=Number(state.profile.budget)||0;
  ...
  function renderAccountMode(){const cloud=Boolean(session);$('#syncText').textContent=cloud?'synced by email':'local preview';$('#accountModeText').textContent=cloud?'Email Magic Link 已登入':'目前是預覽模式';$('#accountTitle').textContent=cloud?'Email account':'Local preview';$('#accountMeta').textContent=cloud?(session.user.email||'已登入'):'資料儲存在這個瀏覽器';$('#accountDot').style.background=cloud?'#7fc38a':'#f0b65b'}
  ...
  function openMealModal(id=null){editingId=id;photoData='';stickerOn=true;$('#stickerToggle').classList.add('active');$('#photoStage').classList.remove('sticker');
    ...
    $('#mealModal').classList.remove('hidden');document.body.style.overflow='hidden';
  }
  function closeMealModal(){ $('#mealModal').classList.add('hidden');document.body.style.overflow='';editingId=null }
  ...
  async function handlePhoto(file){if(!file)return;try{photoData=await compressImage(file,900,.84);showPhoto(photoData);$('#photoStatus').textContent='✦ photo ready';const endpoint=window.FOOD_JAR_CONFIG?.REMOVE_BG_ENDPOINT;if(endpoint){$('#photoStatus').textContent='⋆ AI 去背中…';try{const res=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({image:photoData})});if(res.ok){const data=await res.json();if(data.image){photoData=data.image;showPhoto(photoData);$('#photoStatus').textContent='✦ AI 去背完成'}}else $('#photoStatus').textContent='✦ 使用原圖貼紙'}catch{$('#photoStatus').textContent='✦ 使用原圖貼紙'}}}catch(e){toast('照片讀取失敗，請換一張試試')}}
  function compressImage(file,max=900,quality=.84){return new Promise((resolve,reject)=>{const fr=new FileReader();fr.onerror=reject;fr.onload=()=>{const img=new Image();img.onerror=reject;img.onload=()=>{let w=img.width,h=img.height;if(Math.max(w,h)>max){const r=max/Math.max(w,h);w=Math.round(w*r);h=Math.round(h*r)}const c=document.createElement('canvas');c.width=w;c.height=h;const ctx=c.getContext('2d');ctx.drawImage(img,0,0,w,h);resolve(c.toDataURL('image/jpeg',quality))};img.src=fr.result};fr.readAsDataURL(file)})}

  async function saveMeal(){const wasEditing=Boolean(editingId);const price=Number($('#priceInput').value);if(!price||price<=0){toast('先填上這餐的價格 ♡');$('#priceInput').focus();return}const date=$('#dateInput').value||todayISO();const existing=editingId?state.meals.find(m=>m.id===editingId):null;const meal={id:editingId||`meal-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,date,title:$('#titleInput').value.trim()||$('#typeInput').value,type:$('#typeInput').value,price,payment:$('#paymentInput').value,note:$('#noteInput').value.trim(),image:photoData||existing?.image||'',emoji:existing?.emoji||(!photoData?typeEmoji[$('#typeInput').value]:''),sticker:stickerOn};
    if(editingId){const idx=state.meals.findIndex(m=>m.id===editingId);state.meals[idx]=meal}else state.meals.push(meal);persist();if(session){window.FoodJarCloud.upsertMeal(session.user.id,meal).then(remote=>{const i=state.meals.findIndex(x=>x.id===meal.id);if(i>=0){state.meals[i]=remote;persist();renderAll()}}).catch(()=>toast('已存在本機，雲端稍後再同步'))}selectedMonth=new Date(Number(date.slice(0,4)),Number(date.slice(5,7))-1,1);closeMealModal();renderAll();toast(wasEditing?'已更新這顆糖果 ♡':'已放進糖果罐 🍬')}
  ...
  function bindEvents(){
    ...
    $('#shareCardBtn').onclick=shareCard;$('#exportBtn').onclick=exportData;$('#logoutBtn').onclick=async()=>{if(session)await window.FoodJarCloud.signOut().catch(()=>{});session=null;safeLSRemove(PREVIEW_KEY);showAuth();toast('已登出')};
